...
export const get_local_issue_time = issueTime => {
  return moment.utc(issueTime, timeFormats).local().format('YYYY-MM-DD HH:mm (G[M]T Z)').toString();
};

export const get_local_issue_date = issueTime => {
  if (moment.utc(issueTime, dateFormats).isValid()) {
    return moment.utc(issueTime, timeFormats).local().format('DD MMM YYYY').toString();
  } else {
    let formattedDate = moment(issueTime).format('DD MMM YYYY');
...
export const get_local_date = dateTime => {
  return moment.utc(dateTime).format('YYYY-MM-DD').toString();
};
